...
function _exportPDF(event: EventDetail): void {
  ...
  for (const ch of plan.channels ?? []) {
    ...
    badge(
      doc,
      strip(ch.priority),
      M + 8 + doc.getTextWidth(strip(ch.name)) + 3,
      y + 7,
      pb,
      pc,
    );
    ...
    doc
      ...
      .setTextColor(...C.cyan);
    doc.text(
      (ch.contentTypes ?? []).map(strip).join("  •  "),
      M + 8,
      y + boxH - 5,
    );
    y += boxH + 4;
  ...
  const gap =
    budgets.length > 0
      ? (chW - bW * budgets.length) / (budgets.length + 1)
      : 10;
  ...
  budgets.forEach((item, i) => {
    ...
    doc
      .setFont("helvetica", "bold")
      .setFontSize(7)
      .setTextColor(...BUDGET_COLORS[i % BUDGET_COLORS.length]);
    ...
    cl.forEach((t, ci) =>
      doc.text(t, bx + bW / 2 - doc.getTextWidth(t) / 2, chB + 4 + ci * 4),
    );
  });
  ...
  if ((plan.weeklyContentCalendar ?? []).length > 0) {
    ...
    for (const [wi, week] of (plan.weeklyContentCalendar ?? []).entries()) {
      ...
      for (const post of week.posts ?? []) {
        ...
        let bx = M + 8;
        const by2 = y + 7;
        badge(doc, strip(post.type), bx, by2, C.grayLight, C.gray);
        bx += doc.getTextWidth(strip(post.type)) + 9;
        badge(doc, strip(post.platform), bx, by2, wBg, wCol);
        bx += doc.getTextWidth(strip(post.platform)) + 9;
        doc
          .setFont("helvetica", "italic")
          .setFontSize(7.5)
          .setTextColor(...C.gray);
        ...
        const body = (lines: string[], color: [number, number, number]) => {
          doc
            ...
            .setTextColor(...color);
          lines.forEach((l) => {
            doc.text(l, M + 8, lineY);
            lineY += 4;
          });
          lineY += 2;
        };
        section("VISUAL / CONTENT");
        body(descLines, C.black);
        section("CAPTION");
        body(capLines, C.black);
        section("HASHTAGS");
        body(hashLines, wCol);
