  ...
  return (
    <div className="mt-7 min-h-screen p-4 md:p-6" style={{ background: "#050020" }}>
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        {/* ── Strategy Summary ── */}
        <PlanCard icon={<Target className="h-5 w-5 text-purple-400" />} title="Strategy Overview">
          <p className="leading-relaxed text-gray-300">{plan.summary}</p>
        </PlanCard>

        {/* ── Quick Wins ── */}
        <PlanCard icon={<Zap className="h-5 w-5 text-yellow-400" />} title="Quick Wins — Start Today">
        ...
        {/* ── Marketing Channels ── */}
        <PlanCard icon={<TrendingUp className="h-5 w-5 text-blue-400" />} title="Marketing Channels">
          <div className="space-y-4">
            {plan.channels?.map((ch, i) => (
              <div key={i} className="rounded-xl border border-slate-700/50 bg-slate-900/50 p-4">
                <div className="mb-2 flex items-center justify-between">
                  <span className="font-semibold text-white">{ch.name}</span>
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${PRIORITY_COLORS[ch.priority] ?? PRIORITY_COLORS.Low}`}>
                    {ch.priority}
                  </span>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ch.contentTypes?.map((ct, j) => (
                    <span key={j} className="rounded-md border border-slate-700 bg-slate-800 px-2 py-1 text-xs text-gray-300">
                      {ct}
                    </span>
                  ))}
                </div>
        ...
        {/* ── Timeline ── */}
        <PlanCard icon={<Calendar className="h-5 w-5 text-cyan-400" />} title="Campaign Timeline">
          <div className="space-y-4">
        ...
        {/* ── Budget ── */}
        <PlanCard icon={<DollarSign className="h-5 w-5 text-green-400" />} title="Budget Allocation">
          ...
          <div className="space-y-3">
            {plan.budgetAllocation?.map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className={`mt-1 h-3 w-3 flex-shrink-0 rounded-full ${BUDGET_COLORS[i % BUDGET_COLORS.length]}`} />
        ...
        {plan.weeklyContentCalendar && plan.weeklyContentCalendar.length > 0 && (
          <PlanCard icon={<Hash className="h-5 w-5 text-violet-400" />} title="Weekly Content Calendar">
        ...
        {/* ── Content Ideas ── */}
        <PlanCard icon={<Lightbulb className="h-5 w-5 text-orange-400" />} title="Content Ideas">
        ...
        {/* ── Key Messages ── */}
        <PlanCard icon={<MessageSquare className="h-5 w-5 text-pink-400" />} title="Key Messages">
          <div className="space-y-2">
            {plan.keyMessages?.map((msg, i) => (
              <div key={i} className="flex items-start gap-3 rounded-xl border border-pink-500/20 bg-pink-500/5 p-3">
        ...
        {/* ── Success Metrics ── */}
        <PlanCard icon={<BarChart2 className="h-5 w-5 text-cyan-400" />} title="Success Metrics">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            {plan.successMetrics?.map((metric, i) => (
              <div key={i} className="flex items-center gap-3 rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-3">
