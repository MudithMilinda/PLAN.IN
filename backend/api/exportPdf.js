...
const WEEK_BG       = [C.indigoBg, C.skyBg, C.emeraldBg, C.amberBg, C.roseBg, [245,243,255]];
...
function accentCard(doc, text, accentColor, bgColor, y, x, w) {
  const textH = doc.heightOfString(strip(text), {
    width: w - 20,
    font: "Helvetica",
    size: 9,
  });
...
router.post("/api/export-pdf", (req, res) => {
  try {
    ...
    doc.font("Helvetica").fontSize(9).fillColor("#e0e7ff")
       .text(dateText, margin, 86);

    doc.font("Helvetica-Bold").fontSize(8).fillColor("#a5b4fc")
       .text("LOCATION", margin + 160, 76);
    doc.font("Helvetica").fontSize(9).fillColor("#e0e7ff")
       .text(locationText, margin + 160, 86);

    doc.font("Helvetica-Bold").fontSize(8).fillColor("#a5b4fc")
       .text("AUDIENCE", margin + 320, 76);
    doc.font("Helvetica").fontSize(9).fillColor("#e0e7ff")
       .text(strip(event?.target_audience || ""), margin + 320, 86);
    ...
    for (let i = 0; i < (plan.quickWins || []).length; i++) {
      ...
      const textH  = doc.heightOfString(strip(win), { width: CW - 30, font: "Helvetica", size: 9 });
      const cardH  = textH + 18;
    ...
    for (const ch of (plan.channels || [])) {
      const stratH = doc.heightOfString(strip(ch.strategy || ""), { width: CW - 20, font: "Helvetica", size: 9 });
    ...
    for (let i = 0; i < (plan.timeline || []).length; i++) {
      ...
      const focusH    = doc.heightOfString(strip(phase.focus || ""), { width: CW - 50, font: "Helvetica", size: 9 });
      const tasksText = (phase.tasks || []).map(t => `→  ${strip(t)}`).join("\n");
      const tasksH    = tasksText ? doc.heightOfString(tasksText, { width: CW - 50, font: "Helvetica", size: 9 }) : 0;
...
y += 8;
    ...
    for (let i = 0; i < ideas.length; i += 2) {
      ...
      const heights = pair.map(idea => {
        const h = doc.heightOfString(strip(idea.idea || ""), { width: colW - 16, font: "Helvetica", size: 9 });
        return h + 36;
      });
    ...
    if (weeks.length > 0) {
      ...
      for (let wi = 0; wi < weeks.length; wi++) {
        ...
        for (const post of (week.posts || [])) {
          const descH   = doc.heightOfString(strip(post.contentDescription || ""), { width: CW - 20, font: "Helvetica", size: 9 });
          const capH    = doc.heightOfString(strip(post.caption || ""),             { width: CW - 20, font: "Helvetica", size: 9 });
          const tagsH   = doc.heightOfString(strip(post.hashtags || ""),            { width: CW - 20, font: "Helvetica", size: 8 });
          const cardH   = descH + capH + tagsH + 70;
    ...
    for (let i = 0; i < pageCount; i++) {
      ...
      doc.rect(0, pageH - 24, PW, 24).fill(hex(C.light));
      doc.moveTo(0, pageH - 24).lineTo(PW, pageH - 24)
         .stroke(hex(C.border)).lineWidth(0.5);
      doc.font("Helvetica").fontSize(8).fillColor(hex(C.faint))
         .text(`${strip(event?.event_name || "")} — Marketing Plan`, margin, pageH - 14);
      doc.text(`Page ${i + 1} of ${pageCount}`, 0, pageH - 14, { width: PW - margin, align: "right" });
    }
